...
def load_image(image_path):
    try:
        ...
        raise IOError(f"Could not load image {image_path}: {e}")
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, max_workers=8):
    
    def process_single_camera(key):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_single_camera, key): key for key in keys}
        for future in tqdm(as_completed(futures), total=len(futures), desc="Reading cameras", ncols=80):
            try:
                cam_info = future.result()
                cam_infos.append(cam_info)
            except Exception as e:
                raise RuntimeError(f"Error processing camera {futures[future]}: {e}")
...
def process_camera_info(cam_infos_unsorted, path, reading_dir, max_workers=8):
    ...
    for img in sorted(os.listdir(full_dir)):
        ...
        max_timestamp = max(max_timestamp, int(img.split('.')[0][-4:]))
        image_files[cam_name].append(img)

    print(f'Max timestamp is : {max_timestamp}')
    tasks = []
    uid = max([info.uid for info in cam_infos_unsorted], default=0)
    
    for cam_info in cam_infos_unsorted:
        image_name = cam_info.image_name
        cam_name = image_name.split('/')[-1].split('_')[0]
    ...
    def process_single_image(task):
        try:
            # temp_image = load_image(task['temp_path'])
            temp_image = None
            ...
            temp_info = CameraInfo(
                ...
                R=task['cam_info'].R,
                T=task['cam_info'].T,
                FovY=task['cam_info'].FovY,
                FovX=task['cam_info'].FovX,
                image=temp_image,
                depth=None,
                image_path=task['temp_path'],
                image_name=task['img_name'],
                width=task['cam_info'].width,
                height=task['cam_info'].height,
                timestamp=task['time_stamp'],
                fl_x=task['cam_info'].fl_x,
                fl_y=task['cam_info'].fl_y,
                cx=task['cam_info'].cx,
                cy=task['cam_info'].cy
            )
            return temp_info
            ...
            raise RuntimeError(f"Error processing image {task['temp_path']}: {e}")
    
    new_cam_infos = list(cam_infos_unsorted)
    if tasks:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(process_single_image, task) for task in tasks]
            
            for future in tqdm(as_completed(futures), total=len(futures), 
                             desc="Processing additional images", ncols=80):
                result = future.result()
                if result is not None:
                    new_cam_infos.append(result)
    
    return new_cam_infos

def readColmapSceneInfo(path, images, eval, num_pts_ratio=1.0, 
                        training_cam=['cam10', 'cam01', 'cam20', 'cam13'], testing_cam=None,
                        num_pts=100_000, time_duration=None,
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", time_duration=None, frame_ratio=1, dataloader=False):
    ...
    tbar = tqdm(range(len(frames)), ncols=80, desc="Reading frames from transforms.json")
    def frame_read_fn(idx_frame):
        ...
        if time_duration is not None and 'time' in frame:
            if timestamp < time_duration[0] or timestamp > time_duration[1]:
                print(f"Skipping frame {idx} with timestamp {timestamp} outside time duration {time_duration}.")
                return
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", num_pts=10000000, time_duration=None, num_extra_pts=0, frame_ratio=1, dataloader=False):
